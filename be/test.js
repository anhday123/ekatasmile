...
(async()=>{
    ...
    await new Promise(async(resolve, reject)=>{
        for(let i in taxes) {
            let _tax = new Tax();
            _tax.create(taxes[i]);
            await client.db(DB).collection('Taxes').updateOne({_id: ObjectId(taxes[i]._id)},{$set:_tax});
        }
    ...
    await new Promise(async(resolve, reject)=>{
        for(let i in users) {
            ...
            await client.db(DB).collection('Users').updateOne({_id: ObjectId(users[i]._id)},{$set:_user});
        }
    ...
    await new Promise(async(resolve, reject)=>{
        for(let i in variants) {
            ...
            await client.db(DB).collection('Variants').updateOne({_id: ObjectId(variants[i]._id)},{$set:_variant});
        }
