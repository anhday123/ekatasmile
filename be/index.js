...
localServer.listen(process.env.LOCAL_PORT, () => {
    console.log(
        `Local server runing at http://localhost:${process.env.LOCAL_PORT}/`
    );
});
...
if (opsys != `darwin` && opsys != `win32`) {
    const options = {
        key: fs.readFileSync(
            `/etc/letsencrypt/live/${process.env.GLOBAL_HOST_NAME}/privkey.pem`
        ),
        cert: fs.readFileSync(
            `/etc/letsencrypt/live/${process.env.GLOBAL_HOST_NAME}/cert.pem`
        ),
        ca: fs.readFileSync(
            `/etc/letsencrypt/live/${process.env.GLOBAL_HOST_NAME}/chain.pem`
        ),
    };
