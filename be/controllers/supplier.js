...
let getSupplierC = async (req, res, next) => {
    try {
        if (!valid.relative(req.query, form.getSupplier))
...
let addSupplierC = async (req, res, next) => {
    try {
        ...
        if (!valid.absolute(req.body, form.addsupplier))
            throw new Error(`400 ~ Validate Data Wrong!`);
        req.body[`name`] = String(req.body.name).trim().toUpperCase();
        let [_counts, _count, _bussiness, _supplier] = await Promise.all([
