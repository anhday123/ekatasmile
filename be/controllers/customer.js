  ...
  try {
    ...
    let _customer = {
      ...
      slug_name: removeUnicode(
        String(req.body.first_name + ' ' + req.body.last_name),
        true
      ).toLowerCase(),
      slug_type: removeUnicode(String(req.body.type || ''), true).toLowerCase(),
      slug_gender: removeUnicode(
  ...
  try {
    ...
    rows = rows.map((eRow) => {
      ...
      if (_row['nhomkhachhang']) {
        _row['_nhomkhachhang'] = removeUnicode(
          String(_row['nhomkhachhang']),
          true
        ).toLowerCase()
      ...
      if (eRow['stt']) {
        ...
        let _customer = {
          ...
          type_id: (() => {
            ...
            throw new Error(
              `400: Nhóm khách hàng ${eRow['nhomkhachhang']} không tồn tại!`
            )
          })(),
          ...
          slug_type: removeUnicode(
            String(eRow['_nhomkhachhang'] || ''),
            true
          ).toLowerCase(),
          slug_gender: removeUnicode(
            String(eRow['gioitinh'] || ''),
            true
          ).toLowerCase(),
          slug_address: removeUnicode(
            String(eRow['diachi'] || ''),
          ...
          slug_district: removeUnicode(
            String(eRow['quan/huyen'] || ''),
            true
          ).toLowerCase(),
          slug_province: removeUnicode(
            String(eRow['tinh/thanhpho'] || ''),
            true
          ).toLowerCase(),
        }
