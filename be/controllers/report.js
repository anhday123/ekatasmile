...
module.exports._getIOIReport = async (req, res, next) => {
  try {
    ...
    if (req.query.warehouse_id) {
      exportQuery.push({
        $match: { warehouse_id: Number(req.query.warehouse_id) },
      })
    }
    ...
    aggregateQuery.push(
      {
        $lookup: {
          ...
          pipeline: [
            { $match: { $expr: { $eq: ['$branch_id', '$$branchId'] } } },
          ],
          as: 'branch',
        },
      },
  ...
  try {
    ...
    orders.map((order) => {
      order.order_details.map((detail) => {
        ...
        if (_products[`${detail.product_id}`]) {
          ...
          _products[`${detail.product_id}`].total_revenue +=
            detail.quantity * detail.price
          _products[`${detail.product_id}`].base_price +=
            detail.total_base_price
    ...
    for (let i in _products) {
      ...
      _products[i].profit_rate = Number(
        ((_products[i].total_revenue / _products[i].base_price) * 100).toFixed(
          2
        )
      )
    }
  ...
  try {
    ...
    let [result, counts, total] = await Promise.all([
      ...
      client
        ...
        .aggregate([
          ...countQuery,
          { $group: { _id: { type: '$type' }, total: { $sum: '$value' } } },
        ])
        .toArray(),
    ])
