  ...
  try {
    let pointSetting = await client
      ...
      .findOne({
        $or: [
          { all_branch: true },
          { branch_id: order.sale_location.branch_id },
        ],
      })
    if (!pointSetting) {
      throw new Error(
        `400: Địa điểm bán hàng không áp dụng chương trình tích điểm!`
      )
    }
    let customer = await client
      .db(database)
      .collection('Customers')
      .findOne({ customer_id: order.customer_id })
    if (
      !pointSetting.customer_type_id.includes(customer.type_id) &&
      !pointSetting.all_customer_type
    ) {
    ...
    order.order_details.map((eDetail) => {
      ...
      if (
        pointSetting.all_category ||
        [...new Set(categoryIds)].length != categoryIds.length
      ) {
        increasePoint +=
          (eDetail.price * eDetail.quantity) / pointSetting.exchange_point_rate
      }
    })
    ...
    let pointUseMaxId = await client
      .db(database)
      .collection('AppSetting')
      .findOne({ name: 'PointUseHistories' })
    let pointUseId = (() => {
...
module.exports.enumStatusOrder = async (req, res, next) => {
  try {
    var enums = await client
      .db(DB)
      .collection('EnumStatusOrder')
      .find({})
      .toArray()
  ...
  try {
    ...
    req.body.order_details = req.body.order_details.map((eDetail) => {
      ...
      _detail = {
        ...
        final_cost:
          _detail.price * _detail.quantity - _detail.total_tax ||
          0 - _detail.total_discount ||
          0,
        fulfillment_service: '',
        fulfillment_id: '',
        fulfillment_status: '',
    ...
    await Promise.all(
      _order.payments.map((payment, index) => {
        if (
          payment.method &&
          removeUnicode(String(payment.method).toLowerCase(), true) ==
            'diemtichluy' &&
          !payment.is_used
        ) {
    ...
    if (!/^(draft)$/gi.test(_order.bill_status)) {
      ...
      _order.order_details = _order.order_details.map((eDetail) => {
        ...
        if (detailQuantity > 0) {
          throw new Error(
            '400: Sản phẩm tại địa điểm bán không đủ số lượng cung cấp!'
          )
        }
        return eDetail
      })
      ...
      await client
        ...
        .updateOne(
          { name: 'Finances' },
          { $set: { name: 'Finances', value: receipt_id } },
          { $upsert: true }
        )
...
module.exports._update = async (req, res, next) => {
  try {
    ...
    if (
      ...
      !/^((draft)|(cancel)|(refund))$/gi.test(_order.bill_status)
    ) {
      let sortQuery = (() => {
