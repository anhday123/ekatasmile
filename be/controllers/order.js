  ...
  try {
    ...
    req.body["sale_location"] = await (async () => {
      if (saleAt) {
        let result = await client
          .db(DB)
          .collection(saleAt.collection)
          .findOne(saleAt.location);
        return result;
      }
      return {};
    })();
    ...
    req.body.order_details = req.body.order_details.map((detail) => {
      ...
      for (let i in locationArray) {
        ...
        if (locationArray[i].quantity > 0) {
          if (locationArray[i].quantity > detail.quantity) {
            ...
            total_base_price +=
              _prices[locationArray[i].price_id].import_price * detail.quantity;
            locationArray[i].quantity =
              Number(locationArray[i].quantity) - Number(detail.quantity);
            detail.quantity = 0;
            ...
            total_base_price +=
              _prices[locationArray[i].price_id].import_price *
              locationArray[i].quantity;
            detail.quantity =
              Number(detail.quantity) - Number(locationArray[i].quantity);
      ...
      _detail.create({
        ...
        properties: (() => {
          if (
            _variants[String(detail.variant_id)] &&
            _variants[String(detail.variant_id)].options
          ) {
    ...
    if (
      ...
      if (req.body.voucher && req.body.voucher != "") {
        ...
        if (!promotion) {
          throw new Error(
            "400: Chương trình khuyến mãi không tồn tại hoặc đã hết hạn!"
          );
        }
    ...
    await new Promise(async (resolve, reject) => {
      for (let i in _update) {
        await client
          ...
          .collection("Locations")
          .updateOne(
            { location_id: Number(_update[i].location_id) },
            { $set: _update[i] }
          );
      }
      resolve();
    ...
    await client
      ...
      .updateOne(
        { name: "Orders" },
        { $set: { name: "Orders", value: order_id } },
        { upsert: true }
      );
    req[`_insert`] = _order;
...
let getAllStatusOrderC = async (req, res, next) => {
  try {
    await orderService.getAllStatusOrderC(req, res, next);
  } catch (err) {
    next(err);
  }
};
...
let createStatusOrderC = async (req, res, next) => {
  try {
    if (req.body.data == undefined)
      throw new Error("404: vui lòng truyển mảng data");
    await orderService.createStatusOrderC(req, res, next);
  } catch (err) {
    next(err);
  }
};
...
let getAllReasonRefundOrderC = async (req, res, next) => {
  try {
    await orderService.getAllReasonRefundOrderC (req, res, next);
  } catch (err) {
    next(err);
  }
};
...
let createReasonRefundOrderC = async (req, res, next) => {
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      req.body.data[i].key = removeUnicode(req.body.data[i].label);
      var check = await client
        .db(DB)
        .collection("EnumReasonRefund")
        .findOne({ key: req.body.data[i].key });
      if (check)
        throw new Error(
          `401: lí do ${req.body.data[i].label} đã tồn tại trong hệ thống`
        );
    }

    await orderService.createReasonRefundOrderC(req, res, next);
  } catch (err) {
    next(err);
  }
};
