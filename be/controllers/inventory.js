...
module.exports._createImportOrder = async (req, res, next) => {
  try {
    ...
    req.body.products = req.body.products.map((product) => {
      ...
      final_cost +=
        product.quantity * product.import_price - product.discount || 0
    ...
    let payment_amount = (() => {
      let result = 0
      if (
        Array.isArray(req.body.payment_info) &&
        req.body.payment_info.length > 0
      ) {
    ...
    if (order.status == 'COMPLETE') {
      ...
      let [
        ...
        priceMaxId,
        locationMaxId,
        productInventoryMaxId,
        productInventories,
        variantInventoryMaxId,
        variantInventories,
      ] = await Promise.all([
        client
          .db(req.user.database)
        ...
        client
          .db(req.user.database)
          .collection('AppSetting')
          .findOne({ name: 'ProductInventories' }),
        client
          .db(req.user.database)
        ...
        client
          .db(req.user.database)
          .collection('AppSetting')
          .findOne({ name: 'VariantInventories' }),
        client
          .db(req.user.database)
      ...
      let productInventoryId = (() => {
        if (productInventoryMaxId && productInventoryMaxId.value) {
          return productInventoryMaxId.value
        }
        return 0
      })()
      let variantInventoryId = (() => {
        if (variantInventoryMaxId && variantInventoryMaxId.value) {
          return variantInventoryMaxId.value
        }
        return 0
      })()
      let _prices = {}
      prices.map((ePrice) => {
        _prices[
          `${ePrice.product_id}-${ePrice.variant_id}-${ePrice.import_price}`
        ] = ePrice
      })
      let _productInventories = {}
      productInventories.map((eInventory) => {
        _productInventories[`${eInventory.product_id}`] = eInventory
      })
      let _variantInventories = {}
      variantInventories.map((eInventory) => {
        _variantInventories[`${eInventory.variant_id}`] = eInventory
      })
      let insertPrices = []
      let insertLocations = []
      ...
      order.products.map((eProduct) => {
        if (
          !_prices[
            `${eProduct.product_id}-${eProduct.variant_id}-${eProduct.import_price}`
          ]
        ) {
          priceId++
          let _price = {
        ...
        if (_insertProductInventories[eProduct.product_id]) {
          ...
          _insertProductInventories[eProduct.product_id].end_quantity +=
            eProduct.quantity
          _insertProductInventories[eProduct.product_id].import_quantity +=
            eProduct.quantity * eProduct.import_price
        }
        ...
        if (_insertVariantInventories[eProduct.variant_id]) {
          _productInventories[eProduct.variant_id].import_quantity +=
            eProduct.quantity
          _productInventories[eProduct.variant_id].import_price +=
            eProduct.quantity * eProduct.import_price
      ...
      if (
        Array.isArray(insertVariantInventories) &&
        insertVariantInventories.length > 0
      ) {
        await client
          .db(req.user.database)
          .collection('VariantInventories')
      ...
      if (
        Array.isArray(_updateProductInventories) &&
        _updateProductInventories.length > 0
      ) {
        await Promise.all(
          _updateProductInventories.map((eUpdate) => {
            return client
              ...
              .collection('ProductInventories')
              .updateOne(
                { inventory_id: eUpdate.inventory_id },
                { $set: eUpdate }
              )
          })
        )
      ...
      if (
        Array.isArray(_updateVariantInventories) &&
        _updateVariantInventories.length > 0
      ) {
        await Promise.all(
...
module.exports._createImportOrderFile = async (req, res, next) => {
  try {
        ...
        if (eRow['masanpham'] && eRow['maphienban']) {
          _orders[eRow['maphieunhap']].products.push({
            product_id: (() => {
              ...
              throw new Error(
                `400: Sản phẩm ${eRow['masanpham']} không tồn tại!`
              )
            })(),
            variant_id: (() => {
              ...
              throw new Error(
                `400: Phiên bản ${eRow['maphienban']} không tồn tại!`
              )
            })(),
            import_price: (() => {
              if (eRow['gianhap']) {
              ...
              throw new Error(
                `400: Sản phẩm ${eRow['masanpham']} chưa có giá nhập hàng!`
              )
            })(),
            quantity: (() => {
              if (eRow['soluongnhap']) {
              ...
              throw new Error(
                `400: Sản phẩm ${eRow['masanpham']} chưa có số lượng nhập!`
              )
            })(),
          ...
          _orders[eRow['maphieunhap']].total_cost +=
            eRow['gianhap'] * eRow['soluongnhap']
          _orders[eRow['maphieunhap']].total_tax =
            _orders[eRow['thue(vnd)']] || 0
          _orders[eRow['maphieunhap']].total_discount =
            eRow['chietkhau(vnd)'] || 0
    ...
    await client
      ...
      .updateOne(
        { name: 'ImportOrders' },
        { $set: { name: 'ImportOrders', value: orderId } },
        { upsert: true }
      )
...
module.exports._updateImportOrder = async (req, res, next) => {
  try {
    ...
    let importLocation = await client
      .db(req.user.database)
      .collection('Branchs')
      .findOne(_order.import_location)
    if (!importLocation) {
      throw new Error('400: Địa điểm nhập hàng không chính xác!')
    }
    let productIds = []
    ...
    let payment_amount = (() => {
      ...
      if (
        _order.payment_info &&
        Array.isArray(_order.payment_info) &&
        _order.payment_info.length > 0
      ) {
    ...
    if (_order.status == 'COMPLETE' && order.status != 'COMPLETE') {
      ...
      let [prices, priceMaxId, locationMaxId, inventoryMaxId, inventories] =
        await Promise.all([
          client
            .db(req.user.database)
            .collection('Prices')
      ...
      inventories.map((eInventory) => {
        _inventories[`${eInventory.product_id}-${eInventory.variant_id}`] =
          eInventory
      })
      let insertPrices = []
      let insertLocations = []
      ...
      _order.products.map((eProduct) => {
        ...
        insertLocations.push(_location)
        let _oldInventory =
          _inventories[`${eProduct.product_id}-${eProduct.variant_id}`]
        let _inventory = (() => {
          inventory_id++
          if (!_oldInventory) {
            return {
      ...
      await Promise.all([
        client
          ...
          .updateOne(
            { name: 'Prices' },
            { $set: { name: 'Prices', value: price_id } },
            { upsert: true }
          ),
        client
          .db(req.user.database)
    ...
    await client
      .db(req.user.database)
      .collection('ImportOrders')
      .updateOne(req.params, { $set: _order })
  ...
  try {
    ...
    let [exportLocation, importLocation] = await Promise.all([
      client
        .db(req.user.database)
        .collection(exportAt)
        .findOne(req.body.export_location),
      client
        .db(req.user.database)
        .collection(importAt)
        .findOne(req.body.import_location),
    ])
    if (!exportLocation) {
        ...
        ...(() => {
          return updateLocations.map((eUpdate) => {
            return client
              ...
              .collection('Locations')
              .updateOne(
                { location_id: eUpdate.location_id },
                { $set: eUpdate }
              )
          })
        })(),
...
module.exports._createTransportOrderFile = async (req, res, next) => {
  try {
    ...
    rows = rows.map((eRow) => {
      ...
      _row['_tennoixuathang'] = removeUnicode(
        _row['tennoixuathang'],
        true
      ).toLowerCase()
      _row['_tennoinhaphang'] = removeUnicode(
        _row['tennoinhaphang'],
        true
      ).toLowerCase()
    ...
    rows.map((eRow) => {
      ...
      if (_orders[eRow['maphieuchuyen']]) {
        if (eRow['masanpham'] && eRow['maphienban']) {
          ...
          _orders[eRow['maphieuchuyen']].fee_shipping =
            eRow['phivanchuyen'] || 0
  ...
  try {
    ...
    if (_order.status == 'CANCEL' && order.status != 'CANCEL') {
      ...
      await client
        ...
        .updateOne(
          { name: 'ImportOrders' },
          { $set: { name: 'ImportOrders', value: importOrderId } }
        )
      await client
        .db(req.user.database)
        .collection('ImportOrders')
        .insertOne(importOrder)
    }
    await client
      .db(req.user.database)
      .collection('TransportOrders')
