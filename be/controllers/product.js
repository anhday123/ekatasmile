...
module.exports.addProductC = async (req, res, next) => {
  try {
    ...
    let attributes = await client
      ...
      .toArray();
    let maxAttributeId = await client
      .db(DB)
      .collection("AppSetting")
    ...
    let variants = await client
      ...
      .toArray();
    let maxVariantId = await client
      .db(DB)
      .collection("AppSetting")
      .findOne({ name: "Variants" });
    let maxLocationId = await client
      .db(DB)
      .collection("AppSetting")
      .findOne({ name: "Locations" });
    let _attributeInDBs = {};
    attributes.map((attribute) => {
      _attributeInDBs[`pId${attribute.product_id}-aId${attribute.option}`] =
        attribute;
    });
    let attribute_id = (() => {
      if (maxAttributeId) {
    ...
    req.body.products.map((product) => {
      ...
      if (product.attributes) {
        product.attributes.map((attribute) => {
          ...
          if (
            _attributeInDBs[
              `pId${_product.product_id}-aId${String(
                attribute.option
              ).toUpperCase()}`
            ]
          ) {
      ...
      if (product.variants) {
        product.variants.map((variant) => {
          ...
          if (
            _variantInDBs[
              `pId${_product.product_id}-vId${String(
                variant.sku
              ).toUpperCase()}`
            ]
          ) {
...
module.exports.updateProductC = async (req, res, next) => {
  try {
    ...
    locations.map((location) => {
      _locationInDBs[`vId${location.variant_id}-sId${location.store_id}`] =
        location;
    });
    let location_id = (() => {
      if (maxLocationId) {
    ...
    prices.map((price) => {
      _priceInDBs[
        `vId${price.variant_id}-min${price.min_quantity}-max${price.max_quantity}`
      ] = price;
    });
    let price_id = (() => {
      if (maxPriceId) {
    ...
    if (req.body.attributes) {
      req.body.attributes.map((attribute) => {
        ...
        if (
          _attributeInDBs[
            `pId${_product.product_id}-aId${attribute.option.toUpperCase()}`
          ]
        ) {
          _attribute.create({
            ..._attributeInDBs[
    ...
    if (req.body.variants) {
      req.body.variants.map((variant) => {
        ...
        if (
          _variantInDBs[
            `pId${_product.product_id}-vId${variant.sku.toUpperCase()}`
          ]
        ) {
          _variant.create({
            ..._variantInDBs[
        ...
        if (variant.locations) {
          variant.locations.map((location) => {
            ...
            if (
              _locationInDBs[
                `vId${_variant.variant_id}-sId${location.store_id}`
              ]
            ) {
              _location.create({
                ..._locationInDBs[
...
module.exports.importFileC = async (req, res, next) => {
  try {
    ...
    let excelProducts = XLSX.utils.sheet_to_json(
          ...
          if (product[`Thuộc tính ${i}`]) {
            ...
            if (index != -1) {
              if (
                !_products[product["Mã sản phẩm"]].attributes[
                  index
                ].values.includes(product[`Giá trị ${i}`].toUpperCase())
              ) {
