...
module.exports._register = async (req, res, next) => {
  try {
    ...
    if (
      /^((viesoftware)|(admin)|(login)|(register)|(root))$/gi.test(
        req.body.prefix
      )
    ) {
    ...
    let _admin = {
      ...
      role_id: 1,
      email: req.body.email || "",
      avatar: req.body.avatar || process.env.DOMAIN+'/app/logo.png',
      first_name: req.body.first_name || "",
      last_name: req.body.last_name || "",
      birth_day: req.body.birth_day || moment().tz(TIMEZONE).format(),
      address: req.body.address || "",
    ...
    let _supplier = {
      user_id: 2,
      code: String(0).padStart(6, "0"),
      supplier_id:1,
      name: "Mặc định",
      logo: process.env.DOMAIN+'/app/logo.png',
      phone: "",
      email:"",
      address:"",
      district:"",
    ...
    let _customer = {
      ...
      role_id: 3,
      email: "",
      avatar: process.env.DOMAIN+'/app/logo.png',
      first_name: "Khách lẻ",
      last_name: "",
    ...
    await Promise.all([
      ...
      client
        ...
        .updateOne(
          { name: "Users" },
          { $set: { name: "Users", value: system_user_id } },
          { upsert: true }
        ),
      client
        .db(DB)
      ...
      client
        ...
        .updateOne(
          { name: "Warranties" },
          { $set: { name: "Warranties", value: store_id } },
          { upsert: true }
        ),
    ]);
  ...
  try {
    ...
    let business = await client
      ...
      .aggregate([
        ...
        { $match: { otp_code: req.body.otp_code } },
      ])
      .toArray();
    ...
    if (business.length == 0) {
      throw new Error(
        `400: Tài khoản không tồn tại, mã OTP không chính xác hoặc đã hết hạn sử dụng!`
      );
    }
...
module.exports._recoveryPassword = async (req, res, next) => {
  try {
    ...
    let business = await client
      .db(SDB)
      .collection("Business")
      .findOne({ system_user_id: user.system_user_id });
    const DB = business.database_name;
    await client
      .db(DB)
      .collection("Users")
