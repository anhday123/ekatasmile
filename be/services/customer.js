...
module.exports._get = async (req, res, next) => {
    try {
        ...
        let [customers, counts] = await Promise.all([
            client.db(req.user.database).collection(`Customers`).aggregate(aggregateQuery).toArray(),
            client
...
module.exports._create = async (req, res, next) => {
    try {
        let insert = await client.db(req.user.database).collection(`Customers`).insertOne(req.body);
