...
let getBrandS = async (req, res, next) => {
    try {
        ...
        if (req.query.country_code) {
            aggregateQuery.push({ $match: { country_code: { $eq: String(req.query.country_code).trim().toUpperCase() } } })
        }
        ...
        let [brands, counts] = await Promise.all([
            client.db(DB).collection(`Brands`).aggregate(aggregateQuery).toArray(),
            client
                .db(DB)
                .collection(`Brands`)
...
let createBrandS = async (req, res, next) => {
    try {
        let brand = await client.db(DB).collection(`Brands`).insertOne(req._insert);
        if (!brand.insertedId) {
...
let updateBrandS = async (req, res, next) => {
    try {
        await client.db(DB).collection(`Brands`).updateOne(req.params, { $set: req._update });
