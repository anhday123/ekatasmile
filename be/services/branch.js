      ...
      aggregateQuery.push({
        $match: {
          slug_warehouse_type: new RegExp(
            `${removeUnicode(req.query.warehouse_type, false).replace(
              /(\s){1,}/g,
              '(.*?)'
            )}`,
            'ig'
    ...
    aggregateQuery.push(
      {
        $lookup: {
          ...
          pipeline: [
            { $match: { $expr: { $in: ['$$branchId', '$branch_id'] } } },
          ],
          as: 'point_setting',
        },
      },
      ...
      await Promise.all([
        ...
        client
          ...
          .updateOne(
            ...
            { $set: { branch_id: req.body.branch_id } }
          ),
      ])
    } catch (err) {
      console.log(err)
...
module.exports._update = async (req, res, next) => {
  try {
    ...
    await client
      ...
      .updateMany(
        { branch_id: Number(req.params.branch_id) },
