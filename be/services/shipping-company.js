...
module.exports._get = async (req, res, next) => {
    try {
        ...
        if (req.query.shipping_company_id) {
            aggregateQuery.push({ $match: { shipping_company_id: Number(req.query.shipping_company_id) } });
        }
        if (req.query.code) {
...
module.exports._create = async (req, res, next) => {
    try {
        let insert = await client.db(DB).collection(`ShippingCompanies`).insertOne(req.body);
...
module.exports._update = async (req, res, next) => {
    try {
        await client.db(DB).collection(`ShippingCompanies`).updateOne(req.params, { $set: req.body });
        try {
            let _action = {
                business_id: req.user.business_id,
                type: 'Cập nhật',
