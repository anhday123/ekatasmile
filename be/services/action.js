...
let getActionS = async (req, res, next) => {
    try {
        ...
        if (req.query.properties) {
            mongoQuery['sub_properties'] = new RegExp(removeUnicode(req.query.properties).toLowerCase());
        }
        if (req.query.name) {
        ...
        if (req.query.search) {
            mongoQuery['$or'] = [
                { sub_type: new RegExp(removeUnicode(req.query.search).toLowerCase()) },
                { sub_properties: new RegExp(removeUnicode(req.query.search).toLowerCase()) },
