...
module.exports._get = async (req, res, next) => {
    try {
        ...
        if (req.query.point_setting_id) {
            aggregateQuery.push({ $match: { point_setting_id: Number(req.query.point_setting_id) } });
        }
        ...
        let [settings, counts] = await Promise.all([
            client.db(req.user.database).collection(`PointSettings`).aggregate(aggregateQuery).toArray(),
            client
...
module.exports._create = async (req, res, next) => {
    try {
        let insert = await client.db(req.user.database).collection('PointSettings').insertOne(req.body);
...
module.exports._update = async (req, res, next) => {
    try {
        await client.db(req.user.database).collection(`PointSettings`).updateMany(req.params, { $set: req.body });
        try {
            let _action = {
                type: 'Cập nhật',
                properties: 'Cấu hình tích điểm',
