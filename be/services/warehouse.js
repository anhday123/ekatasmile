...
let getWarehouseS = async (req, res, next) => {
    try {
        ...
        if (req.query.warehouse_id) {
            mongoQuery['warehouse_id'] = req.query.warehouse_id;
        }
        if (token) {
        ...
        let _warehouses = await client.db(DB).collection(`Warehouses`).find(mongoQuery).toArray();
        ...
        // phân trang
        if (page && page_size) {
            _warehouses = _warehouses.slice((page - 1) * page_size, (page - 1) * page_size + page_size);
        }
        ...
        _warehouses.map((_warehouse) => {
            _warehouse[`_business`] = _business[_warehouse.business_id];
            _warehouse[`_creator`] = _creator[_warehouse.creator_id];
            return _warehouse;
        });
        res.send({
            success: true,
...
let addWarehouseS = async (req, res, next) => {
    try {
        let token = req.tokenData.data;
        let _warehouse = await client.db(DB).collection(`Warehouses`).insertOne(req._insert);
        if (!_warehouse.insertedId) throw new Error(`500 ~ Create warehouse fail!`);
        if (token)
...
let updateWarehouseS = async (req, res, next) => {
    try {
        let token = req.tokenData.data;
        await client.db(DB).collection(`Warehouses`).findOneAndUpdate(req.params, { $set: req._update });
        if (token)
            await client.db(DB).collection(`Actions`).insertOne({
                business_id: token.business_id,
                type: `Update`,
