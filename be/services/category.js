...
module.exports._get = async (req, res, next) => {
    try {
        ...
        var [categories, counts] = await Promise.all([
            client.db(req.user.database).collection(`Categories`).aggregate(aggregateQuery).toArray(),
            client
...
module.exports._create = async (req, res, next) => {
    try {
        let insert = await client.db(req.user.database).collection(`Categories`).insertOne(req.body);
        ...
        if (body.condition != undefined) {
            ...
            if (body.condition.must_match == 'all') {
                ...
                console.log(query);
                var data = await client.db(process.env.DB).collection('Products').find(query).toArray();
                    ...
                    for (var j = 0; j < data.length; j++) {
                        for (var i = 0; i < body.condition.function.length; i++) {
                            switch (body.condition.function[i].operator) {
                                case 'starts_with':
                                    if (
                                        new String(data[j][`${body.condition.function[i].name}`]).split('-')[0] ==
                                        body.condition.function[i].value
                                    )
                                        result.push(data[j]);
                                    break;
                                case 'ends_with':
                                    var length = new String(data[j][`${body.condition.function[i].name}`]).split(
                                        '-'
                                    ).length;

                                    if (
                                        new String(data[j][`${body.condition.function[i].name}`]).split('-')[
                                            length - 1
                                        ] == body.condition.function[i].value
                                    )
                                        result.push(data[j]);
                ...
                var result = await client.db(DB).collection('Products').aggregate(query_any).toArray();
...
module.exports._update = async (req, res, next) => {
    try {
        await client.db(req.user.database).collection(`Categories`).updateOne(req.params, { $set: req.body });
        try {
            let _action = {
