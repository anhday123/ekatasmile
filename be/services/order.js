...
let getOrderS = async (req, res, next) => {
    try {
        ...
        if (req.query.order_id)
            mongoQuery = { ...mongoQuery, order_id: req.query.order_id };
        if (token)
            mongoQuery = { ...mongoQuery, bussiness: token.bussiness.user_id };
        if (req.query.employee)
            mongoQuery = { ...mongoQuery, employee: req.query.employee };
        if (req.query.customer)
            mongoQuery = { ...mongoQuery, customer: req.query.customer };
        ...
        if (req.query._customer)
            filterQuery = { ...filterQuery, _customer: req.query._customer };
        if (req.query.status)
        ...
        _orders.map((item) => {
            ...
            _order[
                `_bussiness`
            ] = `${_order.bussiness?.first_name} ${_order.bussiness?.last_name}`;
            _order[
                `_creator`
            ] = `${_order.creator?.first_name} ${_order.creator?.last_name}`;
            _order[
                `_customer`
...
let updateOrderS = async (req, res, next) => {
    try {
        ...
        await client
            .db(DB)
            .collection(`Orders`)
            .updateMany(req.query, { $set: req.body });
        if (token)
            await client.db(DB).collection(`Actions`).insertOne({
