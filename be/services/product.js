  ...
  try {
    ...
    if (req.query.code) {
      aggregateQuery.push({
        $match: {
          code: new RegExp(
            `${removeUnicode(req.query.code, false).replace(
              /(\s){1,}/g,
              "(.*?)"
            )}`,
            "ig"
    ...
    if (req.query.attribute) {
      ...
      filters = filters.map((filter) => {
        let values = filter.values.map((value) => {
          return new RegExp(
            `${removeUnicode(String(value), false).replace(
              /(\s){1,}/g,
              "(.*?)"
            )}`,
            "ig"
        ...
        aggregateQuery.push({
          $match: {
            "attributes.sub_option": new RegExp(
              `${removeUnicode(String(filter.option), false).replace(
                /(\s){1,}/g,
                "(.*?)"
              )}`,
              "ig"
