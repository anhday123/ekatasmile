  ...
  try {
    ...
    if (req.query.sku) {
      aggregateQuery.push({
        $match: {
          sku: new RegExp(
            `${removeUnicode(req.query.sku, false).replace(
              /(\s){1,}/g,
              '(.*?)'
            )}`,
            'ig'
    ...
    if (req.query._deals) {
      aggregateQuery.push({
        $lookup: {
          ...
          let: { productId: '$product_id' },
          pipeline: [
            { $match: { $expr: { $in: ['$$productId', '$product_list'] } } },
          ],
          as: '_deals',
        },
      })
...
module.exports._create = async (req, res, next) => {
  try {
    ...
    let insertProduct = await client
      .db(req.user.database)
      .collection('Products')
      .insertOne(req._product)
  ...
  try {
    ...
    await Promise.all([
      client
        ...
        .collection('Products')
        .updateOne(
          { product_id: req._product.product_id },
          { $set: req._product },
          { upsert: true }
        ),
      Promise.all(
        req._attributes.map((eAttribute) => {
          return client
            ...
            .collection('Attributes')
            .updateOne(
              { attribute_id: eAttribute.attribute_id },
              { $set: eAttribute },
              { upsert: true }
            )
        })
      ...
      Promise.all(
        req._variants.map((eVariant) => {
          return client
            ...
            .collection('Variants')
            .updateOne(
              { variant_id: eVariant.variant_id },
              { $set: eVariant },
              { upsert: true }
            )
        })
    ...
    attributes.map((attribute) => {
      if (!_attributes[attribute.option]) {
        ...
        if (Array.isArray(_attributes[attribute.option].values))
          _attributes[attribute.option].values = _attributes[
            attribute.option
          ].values.concat(attribute.values)
      }
    })
...
module.exports._createFeedback = async (req, res, next) => {
  try {
    let _insert = await client
      .db(req.user.database)
      .collection(`Feedbacks`)
      .insertOne(req._insert)
