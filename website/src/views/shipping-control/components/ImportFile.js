import { Modal, Row, Col, Select, Upload, Button, message, Table, notification } from 'antd'
            ...
            if (info.file.status === 'done') {
                ...
                reader.onload = async (e) => {
                    ...
                    try {
                        ...
                        console.log(e)
                        notification.error({ message: "Thất bại", description: "Upload file thất bại" })
                    }
    ...
    const addToServer = async () => {
        try {
            if (transport) {
                ...
                notification.error({ message: "Thất bại", description: "Vui lòng cho đơn vị vận chuyển" })
            }
        }
        catch (e) {
            console.log(e);
    ...
    return (
        <>
            ...
            >
                <Row>
                    ...
                    <Col span={12}>
                        <Select style={{ width: '100%' }}>
                            {
                                transport.filter(e => e.active).map(e => <Select.Option value={e.transport_id}>{e.name}</Select.Option>)
                            }
                        </Select>
                    </Col>
                </Row>
                <Row>
                    <p>
                        *Chú ý: <br />
                        ...
                        - Chuyển đổi file nhập dưới dạng .XLS trước tải dữ liệu.<br />
                        - Tải file mẫu đơn hàng{' '}
                        <a href="https://ecomfullfillment.s3.ap-southeast-1.amazonaws.com/1629531640659_ecomfullfillment.xlsx" target="_blank">
                            tại đây
                        </a>
                    </p>
                ...
                <Table columns={columns} dataSource={importData} loading={importLoading} scroll={{ x: "max-content" }} />
                <Row justify="end">
                    <Button type="primary" onClick={addToServer}>Lưu</Button>
                </Row>
            </Modal>
        </>
    )
