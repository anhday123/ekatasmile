...
import { Select, Button, Input, Form, Row, Col, DatePicker, Upload, notification } from "antd";
import {
...
import { ArrowLeftOutlined, PlusOutlined, UploadOutlined, InboxOutlined, FileImageOutlined } from "@ant-design/icons";
...
export default function ShippingAdd() {
  ...
  const openNotification = () => {
    notification.success({
      message: 'Thành công',
      description:
        'Thêm thông tin đối tác thành công.',
    });
  };
  ...
  const apiCreateShippingData = async (object) => {
    try {
      ...
      //   openNotificationDeleteSupplier()
      //   setSelectedRowKeys([])
      // }
      // if (res.status === 200) {
      //   openNotification()
    ...
    <>
      <div className={styles["supplier_add"]}>
        <Link className={styles["supplier_add_back_parent"]} style={{ borderBottom: '1px solid rgb(233, 220, 220)', paddingBottom: '1rem' }} to="/shipping/18">
          ...
          <div className={styles["supplier_add_back"]}>Thêm đối tác vận chuyển</div>
        ...
        >
          ...
          <Row style={{ display: 'flex', marginBottom: '1rem', marginTop: '1rem', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
