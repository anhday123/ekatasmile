...
import { Select, Button, Table, Row, Col, Radio, Space, notification } from "antd";
import {
  useHistory,
...
export default function DeliveryUpdate() {
  ...
  const updateDelivery = async () => {
    try {
      const res = await UpdateDelivery(history.location.state.delivery_id, { status })
      if (res.status === 200) {
        notification.success({ message: "Thành công", description: "Cập nhật sản phẩm thành công" })
        history.push('/shipping-product/9')
      }
    }
    catch (e) {
      console.log(e);
      notification.error({ message: "Thất bại!", description: "Cập nhật thất bại" })
    }
  }
  return (
    <>
      <div className={styles["supplier_add"]}>
        ...
        <Row style={{ width: "100%", fontSize: 18, fontWeight: 600, marginBottom: 20 }}>
          Thông tin phiếu chuyển hàng
        </Row>

        <Row gutter={[10, 10]} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
        ...
        <Row style={{ width: '100%' }}>
          <Table style={{ width: '100%' }} columns={columns} dataSource={history.location.state.products} />
        </Row>
        <Row justify="end" style={{ width: '100%', marginTop: 20 }}>
          <Button type="primary" onClick={updateDelivery}>Lưu</Button>
        </Row>
      </div>
    </>
  );
