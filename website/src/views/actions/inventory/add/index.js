...
export default function ProductAdd() {
  ...
  const addOrUpdateProduct = async () => {
    ...
    for (let i = 0; i < variants.length; ++i) {
      ...
      if (!variants[i].locations.length) {
        notification.error({
          message: 'Vui lòng nhập số lượng sản phẩm trong ít nhất 1 chi nhánh!',
        })
        return
      }
    }
    ...
    try {
      ...
      if (isProductHasVariants) {
        ...
        if (!locations.length) {
          notification.error({
            message:
              'Vui lòng nhập số lượng sản phẩm trong ít nhất 1 chi nhánh',
          })
          dispatch({ type: ACTION.LOADING, data: false })
          return
        }
  ...
  const InputQuantityBranch = ({ locations, variant }) => {
    ...
    return (
      <Select
        mode="multiple"
        placeholder="Nhập số lượng vào chi nhánh"
        size="large"
        style={{ width: '100%' }}
        ...
        dropdownRender={() => (
          <div>
            {branches.map((branch) => (
              ...
              >
                <div style={{ color: 'black' }}>{branch.name}</div>
                <Space>
                  <InputNumber
                    min={0}
                    ...
                    onBlur={(e) => {
                      ...
                      const indexLocation = locationsNew.findIndex(
                        (l) => l.inventory_id === branch.branch_id
                      )
  ...
  const columnsVariant = [
    ...
    },
    {
      ...
      width: 250,
      render: (text, record) => (
        <InputQuantityBranch locations={record.locations} variant={record} />
      ),
    },
    {
      title: 'Giá',
